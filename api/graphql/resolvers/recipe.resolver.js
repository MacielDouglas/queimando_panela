    ...
    updateRecipe: async (_, { id, updateRecipe }, { req }) => {
      try {
        ...
        updatableFields.forEach((field) => {
          if (updateRecipe[field] !== undefined && updateRecipe[field] !== "") {
            if (
              ...
              if (
                (Array.isArray(updateRecipe[field]?.ingredients) &&
                  updateRecipe[field].ingredients.length > 0) ||
                (updateRecipe[field]?.ingredients?.length === 1 &&
                  updateRecipe[field].ingredients[0] !== "")
              ) {
        ...
        return {
          ...
          time: recipe.time,
          calda: recipe.calda,
          recheio: recipe.recheio,
          cobertura: recipe.cobertura,
          massa: recipe.massa,
        };
      } catch (error) {
